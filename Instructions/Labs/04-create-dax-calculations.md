...
Introduzca una fórmula DAX válida en la barra de_fórmula_. La barra de fórmulas incluye funciones como autocompletar, Intellisense y codificación por colores, que permiten introducir la fórmula con rapidez y precisión.
...
1. En la tabla`Salesperson`, seleccione las siguientes columnas y, a continuación, ocúltelas (establezca la propiedad**Is Hidden** (Está oculto) en_Sí_):
...
1. En la tabla`Date`, oculte la columna`MonthKey` (establezca**Is Hidden** (Está oculto) en_Sí_).
...
1. En el panel**Propiedades**, en la casilla**Nombre**, sustituya el valor por_Fiscal_.
...
1. En la ventana**Marcar como tabla de fechas**, deslice la propiedad**Marcar como tabla de fechas** a_Sí_.
...
1. Para la selección múltiple de medidas, configure los siguientes requisitos:
    ...
    - Asigne una carpeta de visualización denominada_Precios_ (utilice la propiedad**Carpeta de visualización** del panel**Propiedades**).
...
1. Seleccione varias veces las medidas`Order Lines` y`Orders` y, a continuación, configure los siguientes requisitos:
    ...
    - Asignar a una carpeta para mostrar denominada_Counts_ (Recuentos).
...
1. Cambie el nombre de la columna`Targets | Target` a_TargetAmount_.
